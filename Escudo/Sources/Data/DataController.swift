...
class DataController: ObservableObject {
    ...
    var addedTransaction: Bool {
        get {
            UserDefaults(suiteName: "group.com.sugarhashira.Escudo")!.bool(forKey: "newTransactionAdded")
        }

        set {
            UserDefaults(suiteName: "group.com.sugarhashira.Escudo")!.set(newValue, forKey: "newTransactionAdded")
        }
    }
    ...
    func updateBudgetDates() {
        ...
        budgets.forEach { budget in
            guard budget.type != 5 else { return }
...
//        return Array(Set(notes))
    ...
    func fetchRequestForLogView(type: Int, optionalIncome: Bool?, categoryFilters: [Category] = []) -> NSFetchRequest<Transaction> {
        ...
        if type == 4 {
            ...
            if type == 1 {
                ...
                let startOfMonth = UserDefaults(suiteName: "group.com.sugarhashira.Escudo")!.integer(forKey: "firstDayOfMonth")
    ...
    /// Total income for a calendar month. `offset` 0 = current month, -1 = previous month.
    func getCalendarMonthIncome(offset: Int = 0) -> Double {
        var cal = Calendar(identifier: .gregorian)
        cal.firstWeekday = UserDefaults(suiteName: "group.com.sugarhashira.Escudo")?.integer(forKey: "firstWeekday") ?? 2
        let now = Date.now
        guard let monthStart = cal.date(byAdding: .month, value: offset, to: cal.date(from: cal.dateComponents([.year, .month], from: now))!),
        ...
        req.predicate = NSPredicate(format: "date >= %@ AND date < %@ AND income == true", monthStart as CVarArg, monthEnd as CVarArg)
        let txns = (try? container.viewContext.fetch(req)) ?? []
        return txns.reduce(0) { $0 + $1.amount }
    }

    /// Total expenses for a calendar month. `offset` 0 = current month, -1 = previous month.
    func getCalendarMonthSpent(offset: Int = 0) -> Double {
        var cal = Calendar(identifier: .gregorian)
        cal.firstWeekday = UserDefaults(suiteName: "group.com.sugarhashira.Escudo")?.integer(forKey: "firstWeekday") ?? 2
        let now = Date.now
        guard let monthStart = cal.date(byAdding: .month, value: offset, to: cal.date(from: cal.dateComponents([.year, .month], from: now))!),
              let monthEnd   = cal.date(byAdding: .month, value: 1, to: monthStart) else { return 0 }
        let req: NSFetchRequest<Transaction> = Transaction.fetchRequest()
        req.predicate = NSPredicate(format: "date >= %@ AND date < %@ AND income == false", monthStart as CVarArg, monthEnd as CVarArg)
        let txns = (try? container.viewContext.fetch(req)) ?? []
        return txns.reduce(0) { $0 + $1.amount }
    }
    ...
    /// Last 12 calendar months of expense totals — oldest first, current month last.
    func getLast12MonthsExpenses() -> [Double] {
        let cal = Calendar(identifier: .gregorian)
        let now = Date.now
        return (0..<12).reversed().map { offset -> Double in
            guard let monthStart = cal.date(byAdding: .month, value: -offset, to: cal.date(from: cal.dateComponents([.year, .month], from: now))!),
                  let monthEnd   = cal.date(byAdding: .month, value: 1, to: monthStart) else { return 0 }
            let req: NSFetchRequest<Transaction> = Transaction.fetchRequest()
            req.predicate = NSPredicate(format: "date >= %@ AND date < %@ AND income == false", monthStart as CVarArg, monthEnd as CVarArg)
            return ((try? container.viewContext.fetch(req)) ?? []).reduce(0) { $0 + $1.amount }
        }
    }

    /// Daily expense totals for the last N calendar days (inclusive of today). Oldest first.
    func getDailySpend(lastDays: Int = 35) -> [(date: Date, amount: Double)] {
        let cal = Calendar(identifier: .gregorian)
        let today = cal.startOfDay(for: Date.now)
        let startDate = cal.date(byAdding: .day, value: -(lastDays - 1), to: today) ?? today
        let req: NSFetchRequest<Transaction> = Transaction.fetchRequest()
        req.predicate = NSPredicate(format: "date >= %@ AND income == false", startDate as CVarArg)
        let txns = (try? container.viewContext.fetch(req)) ?? []
        return (0..<lastDays).map { offset in
            let day = cal.date(byAdding: .day, value: offset, to: startDate) ?? startDate
    ...
    /// Per-category spend totals for current calendar month, sorted by total descending.
    /// Returns array of (category, mtdTotal, percentOfTotal, sparkline[7 days]).
    struct CategoryInsightRow {
        let category: Category
        let mtdTotal: Double
        let percent:  Double
        let sparkline: [Double]   // last 7 days, oldest first
    }

    func getCategoryInsights(start: Date? = nil, end: Date? = nil) -> [CategoryInsightRow] {
        let cal = Calendar(identifier: .gregorian)
        let now = Date.now
        ...
        let req: NSFetchRequest<Transaction> = Transaction.fetchRequest()
        req.predicate = NSPredicate(format: "date >= %@ AND date < %@ AND income == false",
                                    rangeStart as CVarArg, rangeEnd as CVarArg)
        let txns = (try? container.viewContext.fetch(req)) ?? []
        let grandTotal = txns.reduce(0) { $0 + $1.amount }
        ...
        return byCategory.map { cat, total in
            let spark: [Double] = (0..<7).reversed().map { dayOffset -> Double in
                ...
                return txns.filter { $0.category == cat && ($0.date ?? now) >= dayStart && ($0.date ?? now) < dayEnd }.reduce(0) { $0 + $1.amount }
            }
            return CategoryInsightRow(category: cat, mtdTotal: total,
                                      percent: total / grandTotal, sparkline: spark)
        }.sorted { $0.mtdTotal > $1.mtdTotal }
    }

    func getSpentInRange(start: Date, end: Date) -> Double {
        let req: NSFetchRequest<Transaction> = Transaction.fetchRequest()
        req.predicate = NSPredicate(format: "date >= %@ AND date < %@ AND income == false",
...
// MARK: - Category UserDefaults flags

extension Category {
    static func usedForExpenseKey(for id: UUID) -> String { "cat_forExpense_\(id.uuidString)" }
    static func usedForIncomeKey(for id: UUID) -> String  { "cat_forIncome_\(id.uuidString)" }

    var usedForExpense: Bool {
        get {
            guard let id = id else { return !income }
            let key = Category.usedForExpenseKey(for: id)
            guard UserDefaults.standard.object(forKey: key) != nil else { return !income }
            return UserDefaults.standard.bool(forKey: key)
        }
        set {
            guard let id = id else { return }
            UserDefaults.standard.set(newValue, forKey: Category.usedForExpenseKey(for: id))
        }
    }

    var usedForIncome: Bool {
        get {
            guard let id = id else { return income }
            let key = Category.usedForIncomeKey(for: id)
            guard UserDefaults.standard.object(forKey: key) != nil else { return income }
            return UserDefaults.standard.bool(forKey: key)
        }
        set {
            guard let id = id else { return }
            UserDefaults.standard.set(newValue, forKey: Category.usedForIncomeKey(for: id))
        }
    }
}

extension DataController {
    func getAllCategoriesForType(isIncome: Bool) -> [Category] {
        let request: NSFetchRequest<Category> = Category.fetchRequest()
        request.sortDescriptors = [NSSortDescriptor(key: "order", ascending: true)]
        let all = results(for: request)
        return all.filter { isIncome ? $0.usedForIncome : $0.usedForExpense }
    }
}
