...
struct BudgetView: View {
    @FetchRequest(sortDescriptors: [SortDescriptor(\.dateCreated)]) private var categories: FetchedResults<Category>
    @FetchRequest(sortDescriptors: [SortDescriptor(\.dateCreated)]) private var budgets: FetchedResults<Budget>
    @FetchRequest(sortDescriptors: [SortDescriptor(\.dateCreated)]) private var mainBudget: FetchedResults<MainBudget>
...
struct ActualBudgetView: View {
    ...
    @AppStorage("budgetViewStyle", store: UserDefaults(suiteName: "group.com.sugarhashira.Escudo")) var budgetRows: Bool = false
                    ...
                    ScrollView(showsIndicators: false) {
                        VStack {
                            if let first = mainBudget.first {
                                EscudoBudgetFuelSection(budget: first)
                                    .id(refreshID)
                            }
...
//            .font(.system(size: 10, weight: .medium, design: .rounded))
    ...
    init(budget: MainBudget, solo: Bool) {
        ...
        _transactions = FetchRequest<Transaction>(sortDescriptors: [SortDescriptor(\.date, order: .reverse)], predicate: andPredicate)
    }
}
...
//                                    .font(.system(size: 11, weight: .semibold, design: .rounded))
    ...
    init(budget: Budget, toDelete: Binding<Budget?>?, toEdit: Binding<Budget?>?, budgetRows: Bool) {
        ...
        let endPredicate = NSPredicate(format: "%K <= %@", #keyPath(Transaction.date), Date.now as CVarArg)
        let incomePredicate = NSPredicate(format: "income = %d", budget.category?.income ?? false)
...
struct AnimatedBudgetBarGraph: View {
    ...
    @AppStorage("animated", store: UserDefaults(suiteName: "group.com.sugarhashira.Escudo")) var animated: Bool = true
...
struct TimeBudgetView: View {
    ...
    var dateString: String {
        ...
        if budgetType == 1 {
            ...
            dateFormatter.dateFormat = "d MMM yy"
            let endDate = budget.customEndDate ?? startDate
            return dateFormatter.string(from: startDate) + " - " + dateFormatter.string(from: endDate)
        } else {
            return ""
        }
            ...
            if let category = budget.category {
                ScrollView(showsIndicators: false) {
                    if budgetType == 1 {
                        ...
                        FilteredBudgetView(category: category, startDate: startDate, totalSpent: $totalSpent, type: budgetType - 1, customEndDate: budgetType == 5 ? budget.customEndDate : nil)
                            .padding(.horizontal, 15)
                    }
                }
                ...
                if budgetType == 5 {
                    // Custom range — show fixed date label, no stepping
                    let fmt: DateFormatter = {
                        let f = DateFormatter(); f.dateFormat = "d MMM yy"; return f
                    }()
                    Text("\(fmt.string(from: budget.wrappedDate)) – \(fmt.string(from: budget.customEndDate ?? budget.wrappedDate))")
...
//        }
    ...
    init(category: Category?, day: Date, totalSpent: Binding<Double>) {
        ...
        let incomePredicate = NSPredicate(format: "income = %d", category?.income ?? false)
...
struct FilteredBudgetView: View {
    ...
    init(category: Category? = nil, startDate: Date, totalSpent: Binding<Double>, type: Int, customEndDate: Date? = nil) {
        date = startDate
        ...
        calendar.firstWeekday = UserDefaults(suiteName: "group.com.sugarhashira.Escudo")!.integer(forKey: "firstWeekday")
        calendar.minimumDaysInFirstWeek = 4

        if let end = customEndDate {
            // Custom date range — include through end of end date
            let nextDay = calendar.date(byAdding: .day, value: 1, to: end) ?? end
            endPredicate = NSPredicate(format: "%K < %@", #keyPath(Transaction.date), nextDay as CVarArg)
        } else if type == 1 {
        ...
        // Guard against nil `day` — NSFetchedResultsController throws if sectionNameKeyPath resolves to nil
        let dayNotNilPredicate = NSPredicate(format: "%K != nil", #keyPath(Transaction.day))

        if let unwrappedCategory = category {
            let categoryPredicate = NSPredicate(format: "%K == %@", #keyPath(Transaction.category), unwrappedCategory)

            let andPredicate = NSCompoundPredicate(type: .and, subpredicates: [startPredicate, endPredicate, categoryPredicate, incomePredicate, dayNotNilPredicate])
            ...
            let andPredicate = NSCompoundPredicate(type: .and, subpredicates: [startPredicate, endPredicate, incomePredicate, dayNotNilPredicate])
...
// MARK: - Grouped income / expense list for the overall budget

struct FilteredBudgetGroupedView: View {
    @FetchRequest private var expenses: FetchedResults<Transaction>
    @FetchRequest private var incomes: FetchedResults<Transaction>
    @Binding var totalSpent: Double
    var date: Date
    ...
    @AppStorage("showCents", store: UserDefaults(suiteName: "group.com.sugarhashira.Escudo")) var showCents: Bool = true
    @AppStorage("swapTimeLabel", store: UserDefaults(suiteName: "group.com.sugarhashira.Escudo")) var swapTimeLabel: Bool = false
    @AppStorage("showExpenseOrIncomeSign", store: UserDefaults(suiteName: "group.com.sugarhashira.Escudo")) var showExpenseOrIncomeSign: Bool = true
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if !expenses.isEmpty {
                ...
                ForEach(expenses, id: \.id) { t in
                    SingleTransactionView(transaction: t, showCents: showCents, currencySymbol: currencySymbol, currency: currency, swapTimeLabel: swapTimeLabel, future: false, showExpenseOrIncomeSign: showExpenseOrIncomeSign)
                }
            }

            if !incomes.isEmpty {
                sectionHeader("INCOME")
                ForEach(incomes, id: \.id) { t in
                    SingleTransactionView(transaction: t, showCents: showCents, currencySymbol: currencySymbol, currency: currency, swapTimeLabel: swapTimeLabel, future: false, showExpenseOrIncomeSign: showExpenseOrIncomeSign)
                }
            }
        }
    ...
    private func recalc() {
        DispatchQueue.main.async {
            totalSpent = expenses.reduce(0) { $0 + $1.wrappedAmount }
        }
    }

    init(startDate: Date, totalSpent: Binding<Double>, type: Int, customEndDate: Date? = nil) {
        date = startDate
        _totalSpent = totalSpent
        ...
        if type == 0 {
            // day
            let dayPred = NSPredicate(format: "%K == %@", #keyPath(Transaction.day), startDate as CVarArg)
            basePredicate = NSCompoundPredicate(type: .and, subpredicates: [dayPred, dateCapPredicate])
        } else {
            let startPredicate = NSPredicate(format: "%K >= %@", #keyPath(Transaction.date), startDate as CVarArg)
            let endPredicate: NSPredicate
            if let end = customEndDate {
                ...
                let next = calendar.isDate(startDate, equalTo: Date.now, toGranularity: .weekOfYear)
                    ? Date.now
                    : (calendar.date(byAdding: .day, value: 7, to: startDate) ?? Date.now)
                endPredicate = NSPredicate(format: "%K < %@", #keyPath(Transaction.date), next as CVarArg)
            } else if type == 2 {
                let next = calendar.isDate(startDate, equalTo: Date.now, toGranularity: .month)
                    ? Date.now
                    : (calendar.date(byAdding: .month, value: 1, to: startDate) ?? Date.now)
                endPredicate = NSPredicate(format: "%K < %@", #keyPath(Transaction.date), next as CVarArg)
            } else {
                let next = calendar.isDate(startDate, equalTo: Date.now, toGranularity: .year)
                    ? Date.now
                    : (calendar.date(byAdding: .year, value: 1, to: startDate) ?? Date.now)
                endPredicate = NSPredicate(format: "%K < %@", #keyPath(Transaction.date), next as CVarArg)
            }
            basePredicate = NSCompoundPredicate(type: .and, subpredicates: [startPredicate, endPredicate, dateCapPredicate])
        }

        _expenses = FetchRequest(
            sortDescriptors: [SortDescriptor(\.date, order: .reverse)],
            predicate: NSCompoundPredicate(type: .and, subpredicates: [basePredicate, NSPredicate(format: "income = false")])
        )
        _incomes = FetchRequest(
            sortDescriptors: [SortDescriptor(\.date, order: .reverse)],
            predicate: NSCompoundPredicate(type: .and, subpredicates: [basePredicate, NSPredicate(format: "income = true")])
        )
    }
}
...
struct TimeMainBudgetView: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            ScrollView(showsIndicators: false) {
                FilteredBudgetGroupedView(
                    startDate: startDate,
                    totalSpent: $totalSpent,
                    type: budgetType == 1 ? 0 : budgetType - 1,
                    customEndDate: budgetType == 5 ? budget.customEndDate : nil
                )
                .padding(.horizontal, 15)
            }
            .frame(maxHeight: .infinity)
