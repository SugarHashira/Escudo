...
struct InsightsView: View {
    ...
    @AppStorage("chartTimeFrame", store: UserDefaults(suiteName: "group.com.sugarhashira.Escudo")) var chartType = 1
...
struct HorizontalPieChartView: View {
    ...
    init(date: Date, categoryFilter: Binding<Category?>?, categoryFilterMode: Binding<Bool>, selectedDate: Binding<Date?>?, chosenAmount: Binding<Double>, chosenName: Binding<String>, type: ChartTimeFrame, income: Bool) {
        ...
        _allCategories = FetchRequest<Category>(sortDescriptors: [SortDescriptor(\.dateCreated)], predicate: NSPredicate(format: "income = %d", income))
...
struct FilteredInsightsView: View {
    ...
    init(startDate: Date, income: Bool? = nil, type: Int) {
        ...
        if let unwrappedIncome = income {
            ...
            andPredicate = NSCompoundPredicate(type: .and, subpredicates: [startPredicate, endPredicate, incomePredicate, dayNotNilPredicate])

        } else {
            andPredicate = NSCompoundPredicate(type: .and, subpredicates: [startPredicate, endPredicate, dayNotNilPredicate])
        }
...
struct SingleGraphView: View {
    ...
    @AppStorage("incomeTracking", store: UserDefaults(suiteName: "group.com.sugarhashira.Escudo")) var incomeTracking: Bool = true
...
//        return abs(offset) > UIScreen.main.bounds.width * 0.3
    ...
    @AppStorage("insightsViewIncomeFiltering", store: UserDefaults(suiteName: "group.com.sugarhashira.Escudo")) var income: Bool = true
    @AppStorage("incomeTracking", store: UserDefaults(suiteName: "group.com.sugarhashira.Escudo")) var incomeTracking: Bool = true
