...
struct TransactionView: View {
    @FetchRequest(sortDescriptors: [SortDescriptor(\.dateCreated)], predicate: NSPredicate(format: "income = %d", false)) private
    var expenseCategories: FetchedResults<Category>
    @FetchRequest(sortDescriptors: [SortDescriptor(\.dateCreated)], predicate: NSPredicate(format: "income = %d", true)) private
    var incomeCategories: FetchedResults<Category>
    @FetchRequest(sortDescriptors: [NSSortDescriptor(keyPath: \Account.dateCreated, ascending: true)]) private
    ...
    @AppStorage("topEdge", store: UserDefaults(suiteName: "group.com.sugarhashira.Escudo")) var topEdge:
    Double = 20
    ...
    @AppStorage(
        "firstTransactionViewLaunch", store: UserDefaults(suiteName: "group.com.sugarhashira.Escudo"))
    ...
    @AppStorage("bottomEdge", store: UserDefaults(suiteName: "group.com.sugarhashira.Escudo"))
    ...
    @AppStorage(
        "showTransactionRecommendations", store: UserDefaults(suiteName: "group.com.sugarhashira.Escudo"))
        ...
        GeometryReader { proxy in
            VStack(spacing: 8) {
                        ...
                        ZStack(alignment: .leading) {
                            ...
                            HStack(spacing: 0) {
                                Text("EXPENSE")
                                    ...
                                    .tracking(1.2)
                                    .lineLimit(1)
                                    .foregroundStyle(income == false ? Color.escudoText : Color.escudoTextMuted)
                                    .padding(6)
                                    .frame(width: capsuleWidth)
                                ...
                                Text("transaction-view-income-picker")
                                    ...
                                    .tracking(1.2)
                                    .lineLimit(1)
                                    .foregroundStyle(income == true ? Color.escudoText : Color.escudoTextMuted)
                                    .padding(6)
                                    .frame(width: capsuleWidth)
                                    .contentShape(Rectangle())
                                    .onTapGesture {
                ...
                ZStack {
                    ...
                    VStack(spacing: 8) {
                        NumberPadTextView(price: $price, isEditingDecimal: $isEditingDecimal, decimalValuesAssigned: $decimalValuesAssigned, accountCurrency: selectedAccount?.currency)
                            ...
                            ForEach(suggestedTransactions, id: \.self) { transaction in
                                Button {
                                    ...
                                    HStack(spacing: 3) {
                                        ...
                                        Text("\(transaction.currencySymbol)\(Int(round(transaction.wrappedAmount)))")
                                            .lineLimit(1)
                    ...
                    HStack(spacing: 8) {
                        ...
                        .overlay(
                            RoundedRectangle(cornerRadius: 11.5, style: .continuous)
                                .strokeBorder(Color.escudoLine, lineWidth: 1)
                        )
                        .contentShape(Rectangle())
                        .onTapGesture {
                        ...
                        if (expenseCategories.count == 0 && !income) || (incomeCategories.count == 0 && income) {
                            ...
                            .overlay(
                                RoundedRectangle(cornerRadius: 11.5, style: .continuous)
                                    .strokeBorder(categoryButtonOutlineColor, lineWidth: 1)
                            )
                            .drawingGroup()
                            .offset(x: shake ? -5 : 0)
                            .contentShape(Rectangle())
...
//                                            .font(.system(.footnote, design: .rounded).weight(.bold))
                                    ...
                                    if let unwrappedCategory = category {
                                        ...
                                        .padding(.vertical, 8.5)
                                        .padding(.horizontal, 10)
                                        .foregroundStyle(Color(hex: unwrappedCategory.wrappedColour))
                                        .background(
                                            Color(hex: unwrappedCategory.wrappedColour).opacity(0.25),
                                            in: RoundedRectangle(cornerRadius: 11.5, style: .continuous)
                                        )
    //                                    .popover(
                    ...
                    if !accounts.isEmpty {
                        HStack(spacing: 0) {
                            Button {
                                ...
                                .padding(.vertical, 7)
                                .padding(.horizontal, 10)
                                .foregroundColor(selectedAccount != nil ? Color(hex: selectedAccount!.colour ?? "007AFF") : Color.SubtitleText)
                                .background(
                                    selectedAccount != nil
                                        ? Color(hex: selectedAccount!.colour ?? "007AFF").opacity(0.2)
                        ...
                        .padding(.bottom, 5)
                        .confirmationDialog("Select Account", isPresented: $showAccountPicker, titleVisibility: .visible) {
                            ForEach(accounts) { acct in
                                Button("\(acct.emoji ?? "🏦") \(acct.name ?? "Account")") {
                                    selectedAccount = acct
                                }
                            }
                            Button("None", role: .destructive) { selectedAccount = nil }
    ...
    func submit() {
        ...
        if let editedTransaction = toEdit {
            ...
            editedTransaction.account = selectedAccount
