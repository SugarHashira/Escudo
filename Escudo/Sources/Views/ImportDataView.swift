...
struct ImportDataView: View {
    ...
    let instructions: [InstructionHeadings] = [
        ...
        InstructionHeadings(title: "Link categories", subtitle: "Match values found in the 'Category' column to the corresponding categories in Escudo."),
...
//                            .font(.system(size: 19, weight: .semibold, design: .rounded))
        ...
        .sheet(isPresented: $showingCategoryView) {
            if #available(iOS 16.0, *) {
                NewCategoryAlert(bottomSpacers: false, budgetMode: false)
                    .presentationDetents([.height(310)])
            } else {
                NewCategoryAlert(bottomSpacers: true, budgetMode: false)
            }
        }
