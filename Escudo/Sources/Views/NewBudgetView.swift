...
struct BrandNewBudgetView: View {
    ...
    @AppStorage("firstWeekday", store: UserDefaults(suiteName: "group.com.sugarhashira.Escudo")) var firstWeekday: Int = 1
    @AppStorage("firstDayOfMonth", store: UserDefaults(suiteName: "group.com.sugarhashira.Escudo")) var firstDayOfMonth: Int = 1
    ...
    @State var customStartDate = Calendar.current.startOfDay(for: Date.now)
    @State var customEndDate = Calendar.current.date(byAdding: .day, value: 30, to: Date.now) ?? Date.now
...
//    }
    ...
    @AppStorage("numberEntryType", store: UserDefaults(suiteName: "group.com.sugarhashira.Escudo")) var numberEntryType: Int = 1
    ...
    var instructions: [InstructionHeadings] {
        [
            ...
            budgetTimeFrame == .custom
                ? InstructionHeadings(title: "Pick date range", subtitle: "Select the start and end dates for this one-time budget.")
                ...
                Button {
                    ...
                    withAnimation(.interactiveSpring(response: 0.6, dampingFraction: 0.8, blendDuration: 0.8)) {
                        if progress < 5 {
                            ...
                            } else if progress == 4 && budgetTimeFrame == .custom {
                                progress += 1
                            } else {
                                progress += 1
                            }
        ...
        .onAppear {
            DispatchQueue.main.async {
                if let unwrappedEditedBudget = toEditBudget {
                    ...
                    case 5:
                        budgetTimeFrame = .custom
                        customStartDate = unwrappedEditedBudget.startDate ?? Calendar.current.startOfDay(for: Date.now)
                        customEndDate = unwrappedEditedBudget.customEndDate ?? Calendar.current.date(byAdding: .day, value: 30, to: Date.now) ?? Date.now
                    default:
                        budgetTimeFrame = .week
                    }
...
//                    }
                ...
                if let unwrappedEditedMainBudget = toEditMainBudget {
                    ...
                    case 5:
                        budgetTimeFrame = .custom
                        customStartDate = unwrappedEditedMainBudget.startDate ?? Calendar.current.startOfDay(for: Date.now)
                        customEndDate = unwrappedEditedMainBudget.customEndDate ?? Calendar.current.date(byAdding: .day, value: 30, to: Date.now) ?? Date.now
                    default:
                        budgetTimeFrame = .week
                    }
...
//                    }
        ...
        .sheet(isPresented: $showingCategoryView) {
            if #available(iOS 16.0, *) {
                NewCategoryAlert(bottomSpacers: false, budgetMode: true)
                    .presentationDetents([.height(310)])
            } else {
                NewCategoryAlert(bottomSpacers: true, budgetMode: true)
            }
        }
    ...
    func submit() {
        ...
        if let unwrappedEditedBudget = toEditBudget {
            ...
            if budgetTimeFrame == .custom {
                unwrappedEditedBudget.customEndDate = Calendar.current.startOfDay(for: customEndDate)
            } else {
                unwrappedEditedBudget.customEndDate = nil
            }
        ...
        if let unwrappedEditedMainBudget = toEditMainBudget {
            ...
            if budgetTimeFrame == .custom {
                unwrappedEditedMainBudget.customEndDate = Calendar.current.startOfDay(for: customEndDate)
            } else {
                unwrappedEditedMainBudget.customEndDate = nil
            }
        ...
        if categoryBudget {
            ...
            newBudget.id = UUID()
            if budgetTimeFrame == .custom {
                newBudget.customEndDate = Calendar.current.startOfDay(for: customEndDate)
            }
        } else {
